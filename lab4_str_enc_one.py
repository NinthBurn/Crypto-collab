...
class MessageConverter:
    def __init__(self, chars: str):
        ...
        self.first_char = self.char_vals[1]
    ...
    def to_str(self, message_num: int) -> str:
        ...
        while message_num > 0:
            ...
            char_index = 1 if char_index == 0 else char_index 
...
class ElGamalEncryption:
    ...
    def encrypt(self, key, message):
        ...
        for chunk in ciphertext_chunks:
            ...
            encrypted_message += self.message_converter.to_str(chunk[0]) + self.message_converter.to_str(chunk[1])
    ...
    def decrypt(self, key, encrypted_message):
        ...
        for i in range(0, len(ciphertext_chunks) - 1, 2):
            alpha, beta = self.message_converter.to_num(ciphertext_chunks[i]) % p, self.message_converter.to_num(ciphertext_chunks[i + 1]) % p
            print(f"original values: {encrypted_message[1][i // 2]}, converted values: {alpha, beta}")
            # alpha, beta = encrypted_message[1][i // 2][0], encrypted_message[1][i // 2][1]
            m = pow(alpha, -a, p) * beta % p
            
            converted_message = self.message_converter.to_str(m)
